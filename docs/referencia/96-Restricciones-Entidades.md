...
| `state_id` valores válidos (1-4) | ✅ Implementada | 🟠 ALTO      | **Validar**: El campo `state_id` solo puede tener valores 1 (registrado), 2 (almacenado), 3 (enviado), o 4 (procesado). **Ya implementado**: Validación mediante constantes en el modelo. **Correcto**: Previene estados inválidos.                                                                                                                        | ok                                                                                                                                                                                                                                                   |
...
| Restricción                                                                      | Estado             | Peligrosidad | Descripción                                                                                                                                                                                                                                                                                                                                                                                                           | Solución/Idea                                                              |
| --------------------------------------------------------------------------------- | ------------------ | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `state_id` ∈ {1, 2, 3, 4}                                                      | ✅ Implementada    | 🟠 ALTO      | **Validar**: El campo `state_id` solo puede tener valores 1, 2, 3 o 4. **Ya implementado**: Validación mediante constantes `STATE_REGISTERED`, `STATE_STORED`, `STATE_SHIPPED`, `STATE_PROCESSED`. **Correcto**: Previene estados inválidos.                                                                                                                                             | ok                                                                          |
| Si `state_id = 2` (almacenado), debe tener `storedPallet`                     | ❌ No implementada | 🔴 CRÍTICO  | **Validar**: Si `state_id = 2`, debe existir un registro `StoredPallet` asociado. **Impacto sin validar**: Permite palets marcados como almacenados sin estar realmente almacenados, generando inconsistencias. **Validación**: Si `state_id = 2`, verificar que `storedPallet` existe.                                                                                                     | Lo trataremos mas adelante                                                  |
| Si `state_id = 3` (enviado), debe tener `order_id`                            | ❌ No implementada | 🟠 ALTO      | **Validar**: Si `state_id = 3`, el campo `order_id` debe tener un valor no nulo. **Impacto sin validar**: Permite palets marcados como enviados sin estar asignados a un pedido, generando inconsistencias. **Validación**: Si `state_id = 3`, verificar que `order_id IS NOT NULL`.                                                                                                        | Lo trataremos mas adelante                                                  |
| Si `state_id = 4` (procesado), todas las cajas deben tener `productionInputs` | ❌ No implementada | 🟠 ALTO      | **Validar**: Si `state_id = 4`, todas las cajas del palet deben tener al menos un `ProductionInput` asociado (fueron consumidas en producción). **Impacto sin validar**: Permite marcar palets como procesados cuando aún tienen cajas disponibles, generando inconsistencias. **Validación**: Si `state_id = 4`, verificar que todas las cajas tienen `productionInputs()->count() > 0`. | Lo trateremos mas adelante                                                  |
...
| Restricción                                              | Estado             | Peligrosidad | Descripción                                                                                                                                                                                                                                                                                                                                                                                                                                            | Solución/Idea        |
| --------------------------------------------------------- | ------------------ | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| No cambiar de `state_id = 4` a otro estado              | ✅ Implementada | 🔴 CRÍTICO  | **Validar**: Si `state_id = 4` (procesado), no se puede cambiar a ningún otro estado (1, 2 o 3). **Impacto sin validar**: Permite revertir palets procesados, generando inconsistencias en la trazabilidad. **Validación**: Al cambiar `state_id`, si el valor actual es 4, impedir el cambio.                                                                                                                                  | ok                    |
| No cambiar de `state_id = 3` a `state_id = 1` o `2` | ✅ Implementada | 🟠 ALTO      | **Validar**: Si `state_id = 3` (enviado), no se puede cambiar a `state_id = 1` (registrado) o `state_id = 2` (almacenado). **Impacto sin validar**: Permite que palets enviados vuelvan a almacén, generando inconsistencias en el flujo de negocio. **Nota**: Puede permitirse cambiar de 3 a 4 (procesado) si se requiere. **Validación**: Al cambiar `state_id`, si el valor actual es 3, impedir cambiar a 1 o 2. | Lo vemos mas adelante |
...
| `pallet.state_id` debe ser `2` (almacenado)       | ❌ No implementada | 🔴 CRÍTICO  | **Validar**: Al crear un `StoredPallet`, el palet asociado debe tener `state_id = 2` (almacenado). **Impacto sin validar**: Permite almacenar palets que no están en estado "almacenado", generando inconsistencias. **Validación**: Antes de crear `StoredPallet`, verificar que `pallet.state_id == 2`.                                                                                      | lo vemos mas adelante                                                                             |
...
| `article_gtin`, `box_gtin`, `pallet_gtin` únicos (si se requieren) | ✅ Implementada | 🟡 MEDIO     | **Validar**: Si los campos `article_gtin`, `box_gtin` o `pallet_gtin` tienen valor, deben ser únicos dentro del tenant. **Impacto sin validar**: Permite GTINs duplicados, causando problemas en sistemas externos que usan estos códigos. **Nota**: Solo aplicar si el sistema requiere GTINs únicos. **Validación**: Si cada campo no es `null`, debe ser único por tenant. **Implementado**: Validación en el modelo `Product` usando evento `saving`. | corregir                 |
...
7. **Pallet Model**:
   - ✅ Validación de `state_id` válido
   - ✅ Bloqueo de cambio de `state_id = 4` (procesado) a otro estado
   - ✅ Bloqueo de cambio de `state_id = 3` (enviado) a `1` o `2`
...
1. **Restricciones que el usuario indicó "No limitar" o "No aplica"**:
   ...
   - `pallet.state_id` debe ser `2` al crear StoredPallet (un palet puede estar vinculado a un pedido pero aún estar en almacén)
...
2. **Restricciones que el usuario indicó "Lo trataremos mas adelante"**:
   - Si `state_id = 2` (almacenado), debe tener `storedPallet`
   - Si `state_id = 3` (enviado), debe tener `order_id`
   - Si `state_id = 4` (procesado), todas las cajas deben tener `productionInputs`
   - No puede tener cajas vacías
   - `box` no debe tener `productionInputs` al agregar a palet
   - `pallet.state_id` debe ser `2` al crear StoredPallet
