...
|---------|------------------|------------------|
| 20 | 20-fundamentos | Introducción, arquitectura, auth, configuración |
| 21 | 21-instrucciones | Deploy, Sail, WSL, guías paso a paso |
| 22 | 22-pedidos | Módulo pedidos |
| 23 | 23-inventario | Almacenes, palets, cajas, stock |
| 24 | 24-catalogos | Productos, especies, clientes, proveedores, etc. |
| 25 | 25-produccion | Módulo producción (y subcarpetas analisis, cambios, frontend si existen) |
| 26 | 26-recepciones-despachos | Recepciones MP, despachos, liquidación |
| 27 | 27-etiquetas | Sistema de etiquetas |
| 28 | 28-sistema | Usuarios, roles, sesiones, logs, control horario |
| 29 | 29-utilidades | PDF, Excel, IA, etc. |
| 30 | 30-referencia | Modelos, rutas, errores, glosario |
| 31 | 31-api-references | Referencia API por módulo (README por área) |
| 32 | 32-ejemplos | Ejemplos de respuestas JSON |
| 33 | 33-frontend | Guías de integración frontend |
| 34 | 34-por-hacer | Backlog, tareas pendientes |
| 35 | 35-prompts | Prompts de agentes IA (auditoría, seeders, etc.) |

- Si el proyecto tiene menos carpetas, se usan solo los números que correspondan (ej. 20, 21, 24, 25, 30, 31).
- Si aparece una carpeta que no está en la tabla (ej. «analisis» suelta en raíz de docs), se integra en la carpeta adecuada (ej. 25-produccion/analisis) o se asigna el siguiente número disponible.
- Carpetas vacías: eliminar. Carpetas ya numeradas (11-DEPLOYMENT, 12-TROUBLESHOOTING, 13-POSTMORTEMS, 14-ARCHITECTURE-DECISIONS) **no** se renumeran; solo las carpetas por dominio sin prefijo.
...
1. **Listar** todas las carpetas bajo `docs/` que sean de contenido por dominio (fundamentos, instrucciones, catalogos, produccion, pedidos, inventario, recepciones-despachos, sistema, utilidades, referencia, API-references, ejemplos, frontend, etiquetas, por-hacer, prompts, etc.) y que **no** tengan ya prefijo 11–16 (11-DEPLOYMENT, 12-TROUBLESHOOTING, etc.).

2. **Renombrar** cada una con el prefijo correspondiente según la tabla de la sección 4.1 (20-fundamentos, 21-instrucciones, 22-pedidos, …, 35-prompts). Ajustar el mapeo al inventario real del proyecto.

3. **Actualizar todos los enlaces** en el repositorio que referencien las rutas antiguas:
   - En todos los `.md` de `docs/`: reemplazar `](./carpeta/`, `](../carpeta/`, `](../../carpeta/`, `](carpeta/` por la nueva ruta con prefijo (ej. `](./20-fundamentos/`).
   - Reemplazar también referencias con ruta absoluta desde raíz: `docs/carpeta/` → `docs/20-fundamentos/` (y equivalentes) en todos los `.md` del proyecto, incluido el README de la raíz.

4. **Actualizar índices:** `docs/00-OVERVIEW.md` y `docs/README.md` deben reflejar la nueva estructura (carpetas 20–35) y todos los enlaces a esas carpetas.
...
- [ ] Renombrar cada carpeta al prefijo 20–35 según sección 4.1 (ej. fundamentos → 20-fundamentos, instrucciones → 21-instrucciones, …)
- [ ] Actualizar en todos los .md: enlaces relativos \`](./carpeta/\`, \`](../carpeta/\`, \`](carpeta/\` y rutas \`docs/carpeta/\` a las nuevas rutas con prefijo
- [ ] Actualizar docs/00-OVERVIEW.md y docs/README.md con la estructura 20–35
...
- [ ] Renombrar \`docs/API.md\` → \`docs/08-API-REST.md\` (si aplica)
...
### MOVIMIENTOS
- [ ] (Mover documentos sueltos a 12-TROUBLESHOOTING, 13-POSTMORTEMS, etc. según mapeo)

### FUSIONES
- [ ] (Fusionar documentos duplicados según mapeo)

### DEPRECACIONES
- [ ] (Deprecar con banner o mover a 13-POSTMORTEMS según mapeo)

## 🟡 ACCIONES ALTAS (SEMANA 2)

### CREACIÓN DE CARPETAS NUEVAS (si aplica)
- [ ] Crear \`docs/16-OPERATIONS/\` solo si existe contenido
- [ ] (14-ARCHITECTURE-DECISIONS, 13-POSTMORTEMS si no existen)
...
- [ ] Actualizar docs/00-OVERVIEW.md y docs/README.md con mapa completo (carpetas 20–35)
...
8. ✋ **Obligatorio:** Carpetas de dominio bajo `docs/` deben renombrarse con prefijo 20–35 (sección 4.1) y actualizarse **todos** los enlaces en el repo (`.md`, README raíz, 00-OVERVIEW, docs/README) antes de dar por cerrada la reestructuración.
...
* ✅ **Carpetas por dominio renombradas con prefijo 20–35** (sección 4.1); todos los enlaces actualizados (docs/, README raíz, 00-OVERVIEW, docs/README); sin referencias a rutas antiguas
...
✅ **Carpetas por dominio numeradas 20–35** — Renombrar obligatoriamente (20-fundamentos, 21-instrucciones, …) y actualizar todos los enlaces en el repo
