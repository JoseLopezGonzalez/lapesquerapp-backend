...
2. **Añadir cabeceras CORS en Nginx** (recomendado si el problema persiste):
   - Ver **`docs/21-instrucciones/nginx-cors-api.conf`** para el `map` de orígenes permitidos y los bloques `add_header ... always` dentro del `location /api/`.
...
| `docs/21-instrucciones/nginx-cors-api.conf` | Snippet Nginx para CORS en API |
| `docs/21-instrucciones/CORS-proxy-Origin.md` | Diagnóstico y proxy + SW |
| `docs/21-instrucciones/CORS-SOLUCION-COMPLETA.md` | Este resumen |
