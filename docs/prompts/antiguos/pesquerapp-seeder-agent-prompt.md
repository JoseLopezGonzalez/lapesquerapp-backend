...
Analizar una copia de seguridad de base de datos MySQL de un tenant de PesquerApp en producción y generar un conjunto de seeders Laravel Faker-based que **repliquen estructuras y datos reales con máxima fidelidad**, garantizando instancias activas y **variedad de variantes por entidad** (un poco de todo) para que el entorno de desarrollo se sienta real y permita pruebas muy realistas.

**IMPORTANTE — Lo que SÍ debe hacerse:**
- **Leer y parsear el backup SQL** (CREATE TABLE + INSERT) para extraer esquema y **valores reales** (estados, códigos, nombres, proporciones).
- **Extraer de los INSERT**: valores distintos por columna, conteos por estado/tipo, rangos de fechas y volúmenes; usar estos datos (o análogos) como base de los seeders.
- **Generar o reescribir seeders** que reflejen esas variantes: todas las variantes de cada entidad representadas (estados de pedido, de palé, tipos de producto, etc.), no solo “unos pocos” genéricos.

**Lo que NO sustituye este objetivo:** Hacer solo idempotentes los seeders existentes sin analizar el backup ni extraer datos reales. El entregable son seeders **realistas y completos derivados del backup**, no únicamente ajustes de re-ejecución.
...
1. **Leer el backup SQL** y extraer:
   * Todas las tablas y sus columnas (CREATE TABLE)
   * Tipos de datos y restricciones
   * Relaciones (FK, índices)
   * Tablas de referencia (lookups, enumeraciones)
   * **De los INSERT: valores reales** — para cada tabla relevante, muestrear o agregar: valores distintos por columna (ej. `status`, `type`, códigos), conteos por estado/tipo, rangos (fechas, cantidades, pesos). Este paso es obligatorio: los seeders deben basarse en estos valores/proporciones.
...
4. **Generar documento de análisis** (guardar en `01_analysis/`):
   ...
   ## Variantes extraídas del backup (obligatorio)
   [Por entidad: valores distintos de estados/tipos/códigos encontrados en los INSERT; proporciones aproximadas; rangos de fechas/volúmenes. Los seeders deben incluir "un poco de todo" según esto.]
   ```
...
1. **Crear o reescribir archivo seeder para cada entidad principal**:
   * **Usar las variantes y valores extraídos en FASE 1** (estados, tipos, códigos, proporciones del backup); no inventar solo valores genéricos.
   * Usar Laravel Faker en español cuando sea posible para datos complementarios
   * Aplicar reglas de negocio del dominio pesquero
   * Mantener consistencia de relaciones
   * Objetivo: "un poco de todo" por entidad (todas las variantes representadas) para pruebas realistas.
2. **Inyectar realismo específico del dominio** (y del backup cuando aplique):
   ```php
