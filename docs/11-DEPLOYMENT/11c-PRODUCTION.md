...
- **Secretos:** variables de entorno de producción configuradas (APP_KEY, DB_*, REDIS_*, mail, etc.). No usar valores de desarrollo. Ver [../02-ENVIRONMENT-VARIABLES.md](../02-ENVIRONMENT-VARIABLES.md).
...
- **Colas y scheduler:** en producción suelen usarse un driver de cola (redis/database) y un cron que ejecute `schedule:run`. Ver [../05-QUEUES-JOBS.md](../05-QUEUES-JOBS.md) y [../06-SCHEDULER-CRON.md](../06-SCHEDULER-CRON.md) cuando estén completos.
...
- **Scheduler:** que el cron esté llamando a `schedule:run` y que los comandos programados (p. ej. limpieza de PDFs, tokens) se ejecuten según [../06-SCHEDULER-CRON.md](../06-SCHEDULER-CRON.md).
...
Documentación de observabilidad: [../10-OBSERVABILITY-MONITORING.md](../10-OBSERVABILITY-MONITORING.md). Runbook operativo: [11e-RUNBOOK.md](./11e-RUNBOOK.md).
...
- [../15-MULTI-TENANT-SPECIFICS.md](../15-MULTI-TENANT-SPECIFICS.md) — Multi-tenant y despliegue.
