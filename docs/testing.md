...
**Total: 88+ tests** (ejecuta `php artisan test` para ver el resultado actual).
...
Para tests de API v2: usar `$this->get('/api/v2/...')` o `$this->post(...)` con headers (p. ej. `X-Tenant`, `Authorization`) según [api-rest.md](./api-rest.md) y [fundamentos/02-Autenticacion-Autorizacion.md](./fundamentos/02-Autenticacion-Autorizacion.md).
...
- [setup-local.md](./setup-local.md) — Entorno local y Sail.
- [api-rest.md](./api-rest.md) — API v2 (endpoints a probar).
- [environment-variables.md](./environment-variables.md) — Variables que afectan a tests (DB_DATABASE, etc.).
- [troubleshooting/COMMON-ERRORS.md](./troubleshooting/COMMON-ERRORS.md) — Errores comunes.
