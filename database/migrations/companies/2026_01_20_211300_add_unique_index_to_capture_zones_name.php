...
return new class extends Migration
{
     ...
     * múltiples zonas de captura con el mismo nombre dentro del mismo tenant.
     */
    public function up(): void
    {
        if (!Schema::hasTable('capture_zones')) {
            return;
        }
        ...
        try {
            $indexes = \DB::select("SHOW INDEX FROM capture_zones WHERE Key_name != 'PRIMARY'");
        ...
        if (!$hasNameUnique) {
            Schema::table('capture_zones', function (Blueprint $table) {
                try {
                    $table->unique('name');
                } catch (\Exception $e) {
