...
class UpdateRawMaterialReceptionRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if ($reception->creation_mode === RawMaterialReception::CREATION_MODE_LINES || $reception->creation_mode === null) {
            $base['details'] = 'nullable|array|min:1';
            $base['details.*.product.id'] = 'required_with:details|exists:tenant.products,id';
            $base['details.*.netWeight'] = 'required_with:details|numeric|min:0';
        ...
        if ($reception->creation_mode === RawMaterialReception::CREATION_MODE_PALLETS) {
            ...
            $base['pallets.*.boxes.*.product.id'] = 'required_with:pallets|exists:tenant.products,id';
            $base['pallets.*.boxes.*.lot'] = 'nullable|string|max:255';
            $base['pallets.*.boxes.*.gs1128'] = 'required_with:pallets|string|max:255';
            $base['pallets.*.boxes.*.grossWeight'] = 'required_with:pallets|numeric|min:0';
            $base['pallets.*.boxes.*.netWeight'] = 'required_with:pallets|numeric|min:0.01';
            $base['prices'] = 'required_with:pallets|array|min:1';
