...
Route::group(['prefix' => 'v2', 'as' => 'v2.', 'middleware' => ['tenant']], function () {
    ...
    Route::middleware(['auth:sanctum', 'external.active', 'actor:internal,external', 'blocklist.email'])->group(function () {
        ...
        // Búsqueda por lote: debe ir antes de cualquier {pallet} para evitar que la capture la ruta comodín
        ...
        // Adjuntos de pedidos
        Route::get('orders/{order}/attachments', [OrderAttachmentController::class, 'index']);
        Route::post('orders/{order}/attachments', [OrderAttachmentController::class, 'store']);
        Route::get('orders/{order}/attachments/{attachment}', [OrderAttachmentController::class, 'show']);
        Route::patch('orders/{order}/attachments/{attachment}', [OrderAttachmentController::class, 'update']);
        Route::get('orders/{order}/attachments/{attachment}/download', [OrderAttachmentController::class, 'download']);
        Route::get('orders/{order}/attachments/{attachment}/thumbnail', [OrderAttachmentController::class, 'thumbnail']);
    ...
    Route::middleware(['auth:sanctum', 'external.active', 'role:tecnico,administrador,direccion,administracion,comercial,operario,supervisor', 'blocklist.email'])->group(function () {
        ...
        Route::delete('sessions', [SessionController::class, 'destroyMultiple'])->name('sessions.destroy_multiple');
        ...
        // Rutas especiales de pallets: disponibles para pedido
